...
test("records readable ordered actions without opaque IDs", () => {
  ...
  recordAction(s, {
    label: "Inspect restoration cart",
    branch: "access",
    discovery: "Fresh plaster grit at office threshold",
  });
  recordAction(s, {
    label: "Press Julian",
    branch: "statements",
    discovery: "Julian denies entering the office",
  });
  assert.equal(
    s.actionSequence.join(" > "),
    "Inspect restoration cart > Press Julian",
  );
  assert.deepEqual(s.investigationBranches, ["access", "statements"]);
  assert.deepEqual(s.discoveries, [
    "Fresh plaster grit at office threshold",
    "Julian denies entering the office",
  ]);
});
...
test("telemetry exports one-row friendly strings and representative-test signals", () => {
  ...
  s.pulseResult = "win";
  s.caseCommitment =
    "killer=Julian | mechanism=Substituted dose | access=Restoration key | failed_claim=Never entered office";
  const row = serializeTelemetry(s, {
    deviceClass: "mobile",
    browserClass: "Chrome",
  });
...
test("feedback URL maps all telemetry keys to the observed Jotform names", () => {
  ...
  const telemetry = Object.fromEntries(
    Object.keys(map).map((key) => [key, `value-${key}`]),
  );
...
test("filters fixture choices that depend on undiscovered evidence", () => {
  ...
  assert.deepEqual(
    filterEligibleOptions(options, ["cart"]).map((x) => x.label),
    ["Use office access", "Keep it private"],
  );
});
