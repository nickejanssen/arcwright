...
**Interaction profiles:** Facilitated live operation plus Decision interview.

Every rehearsal or playtest follows separate preparation, preflight,
founder-walkthrough, explicit go or no-go, guided live run, debrief, and
remediation-approval phases. Before each pause, explain what the founder is
seeing, how to inspect it, what to record, and which failures block progress.
Approval is per run and cannot be carried forward after environment or scope
changes.

AW-269 cloud choices and owner-only actions use Decision interview. Research the
constraints, present two or three viable options with a recommendation, ask one
focused interactive choice question at a time, and stop before external actions,
billing changes, or credential-dependent work. Guide owner actions without
requesting secrets in chat and preserve only redacted completion evidence.

Record preflight evidence, walkthrough feedback, explicit decisions, dates,
debrief findings, remediation approval, and outstanding owner actions.
