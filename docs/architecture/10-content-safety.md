...
The policy block has two arc-sourced parts: plain-language lines derived from `content_rails.prohibited_categories`, and the arc's `content_rails.extra_prohibitions` sentences appended verbatim. Experience-specific policy text always lives in the arc definition, never in engine code, so the engine stays game-agnostic. When no content rails are supplied, the engine injects a platform-minimum block mirroring the four L1 hard-stop categories so L3 always runs.
