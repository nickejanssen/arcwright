...
- Provider or model strings outside `config/routing_table.json` and `engine/routing/router.py`. Enforce this exactly as `AGENTS.md` states the provider-agnostic rule (a violation is a bug); do not invent exceptions to it. The `no_hardcoded_model_strings_outside_routing_layer` eval is the automated check and runs over `engine/`, `api/`, `config/`, and `.github/workflows/`. Beyond what the eval covers, also flag the per-task model mapping being copied anywhere (it will drift) and ask for a pointer to `config/routing_table.json` instead of a copy.
