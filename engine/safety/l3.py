...
Every rule in the block is derived from the arc's `content_rails`
configuration (`prohibited_categories` plus arc-authored
`extra_prohibitions` sentences), so tests can prove the policy is sourced
from the arc, not baked into the platform.
...
def build_l3_policy_block(content_rails: "ContentRailsConfig") -> str:
    ...
    instructions, then appends any arc-authored `extra_prohibitions`
    sentences verbatim.  Every rule in the block is sourced from the arc's
    `content_rails` configuration, never from engine constants, so any game
    can define its own L3 rules without engine code changes.
    ...
    arc_prohibitions.extend(content_rails.extra_prohibitions)
